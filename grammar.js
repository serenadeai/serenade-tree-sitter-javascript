module.exports = grammar({
  ...
  rules: {
    ...
    export_clause: ($) =>
      seq("{", commaSep(alias($._import_export_specifier, $.export_specifier)), optional(","), "}"),
    ...
    for_clause: ($) =>
      seq(
        "for",
        "(",
        field("block_initializer_optional", choice($.block_initializer, $.empty_statement)),
        field(
          "condition_optional",
    ...
    break_statement: ($) =>
      seq("break", optional(alias($.identifier, $.statement_identifier)), $._semicolon),

    continue_statement: ($) =>
      seq("continue", optional(alias($.identifier, $.statement_identifier)), $._semicolon),
    ...
    throw_statement: ($) => seq(field("throw", seq("throw", $._expressions)), $._semicolon),
    ...
    case: ($) => seq("case", field("value", $._expressions), ":", repeat($.statement)),

    switch_default: ($) => seq("default", ":", repeat($.statement)),

    catch: ($) =>
      seq("catch", optional(seq("(", $.catch_parameter, ")")), field("body", $.enclosed_body)),

    catch_parameter: ($) => choice($.identifier, $._destructuring_pattern),

    finally_clause: ($) => seq("finally", field("body", $.enclosed_body)),

    parenthesized_expression: ($) => seq("(", $.parenthesized_expression_inner, ")"),
    ...
    assignment_initializer: ($) => seq("=", field("assignment_value", $.expression)),
    ...
    arguments: ($) =>
      seq("(", optional_with_placeholder("argument_list", commaSep(optional($.argument))), ")"),
    ...
    formal_parameters: ($) =>
      seq(
        "(",
        optional_with_placeholder("parameter_list", seq(commaSep1($.parameter), optional(","))),
        ")"
      ),
    ...
    rest_pattern: ($) => seq("...", choice($.identifier, $._destructuring_pattern)),
